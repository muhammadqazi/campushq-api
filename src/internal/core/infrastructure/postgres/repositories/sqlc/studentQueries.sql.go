...
const selectAllStudents = `-- name: SelectAllStudents :many
SELECT student_id, first_name, surname, sex, role, status, access_status, acceptance_type, graduation_date, supervisor_id, department_id, created_at, updated_at, deleted_at, is_active FROM students ORDER BY student_id DESC LIMIT $1 OFFSET $2
`

type SelectAllStudentsParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

func (q *Queries) SelectAllStudents(ctx context.Context, arg SelectAllStudentsParams) ([]Student, error) {
	rows, err := q.db.Query(ctx, selectAllStudents, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
